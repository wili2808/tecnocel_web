...
export const FavoritosGlobalProvider: React.FC<FavoritosGlobalProviderProps> = ({ children }) => {
  ...
  const removeAllFavoritos = useCallback(
    async (productIds: number[]): Promise<boolean> => {
      ...
      try {
        ...
        const removePromises = productIds.map((productId) => favoritoService.removeFavorito(user.id, productId));
        ...
        setState((prev) => {
          ...
          return {
            ...prev,
            favoritos: newFavoritos,
            favoritosCompletos: prev.favoritosCompletos.filter((f) => !productIds.includes(f.id_producto)),
            lastUpdated: Date.now(),
          };
        });
  ...
  return <FavoritosContext.Provider value={contextValue}>{children}</FavoritosContext.Provider>;
};
