...
export const OfertasGlobalProvider: React.FC<OfertasGlobalProviderProps> = ({ children }) => {
  ...
  const loadOfertas = useCallback(async () => {
    ...
    try {
      ...
      if (import.meta.env.DEV) {
        console.log(
          `✅ Ofertas cargadas desde servidor: ${ofertasData.length} ofertas, ${productosData.data.length} productos`,
        );
      }
    } catch (error) {
      if (import.meta.env.DEV) {
   ...
   */
  const getProductosPorOferta = useCallback(
    (ofertaId: number): Product[] => {
   ...
   */
  const isProductoEnOferta = useCallback(
   ...
   */
  const getOfertaInfo = useCallback(
    (productId: number): OfertaConInfo | null => {
      const producto = state.cache.productos.get(productId);
   ...
   */
  const buscarOfertas = useCallback(
    async (criterios: {
      nombre?: string;
      tipoDescuento?: 'porcentaje' | 'monto_fijo';
      activo?: boolean;
  ...
  return <OfertasContext.Provider value={contextValue}>{children}</OfertasContext.Provider>;
};
