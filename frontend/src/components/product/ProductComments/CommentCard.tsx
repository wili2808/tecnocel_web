...
const CommentCard: React.FC<CommentCardProps> = ({
  ...
  const renderStars = (rating?: number) => {
    ...
    return (
      <div className={styles.rating}>
        {[1, 2, 3, 4, 5].map((star) => (
          <span key={star} className={`material-icons ${styles.star} ${star <= rating ? styles.active : ''}`}>
            {star <= rating ? 'star' : 'star_border'}
          </span>
        ))}
        <span className={styles.ratingText}>{commentService.generarTextoCalificacion(rating)}</span>
      </div>
    );
  };
    ...
    return (
      <div className={styles.editableRating}>
        {[1, 2, 3, 4, 5].map((star) => (
          ...
          >
            <span className="material-icons">{star <= (rating || 0) ? 'star' : 'star_border'}</span>
          </button>
        ))}
      </div>
    );
  ...
  const activeExpandedImage =
    expandedImageIndex !== null && expandedImageList[expandedImageIndex] ? expandedImageList[expandedImageIndex] : null;
        ...
        <div className={styles.userInfo}>
          ...
          <div className={styles.userDetails}>
            <div className={styles.userName}>
              ...
              {isSystemUser && comentario.estado === 'oculto' && <span className={styles.hiddenBadge}>OCULTO</span>}
            </div>
        ...
        {!isCurrentlyEditing ? (
          ...
          <div className={styles.editSection}>
            <textarea
              ...
              disabled={isSubmitting}
            />
            <div className={styles.charCount}>{editText.length}/2000 caracteres</div>
          </div>
        )}
      </div>
      ...
      <div className={styles.commentImages}>
              ...
              return (
                <div key={`new-${index}`} className={styles.imageContainer}>
                  <button
                    type="button"
                    className={styles.imagePreviewButton}
                    onClick={() => openImageModal([{ src: previewUrl, alt: `Nueva imagen ${index + 1}` }], 0)}
                    aria-label="Abrir imagen en tamaño completo"
                  >
                    <img src={previewUrl} alt={`Nueva imagen ${index + 1}`} className={styles.commentImage} />
                  </button>
                  <button
                    type="button"
                    className={styles.deleteImageButton}
        ...
        {isCurrentlyEditing && (
          <div className={styles.addImageContainer}>
            ...
            <div className={styles.imageCount}>{(comentario.imagenes?.length || 0) + newImages.length}/5 imágenes</div>
          </div>
        )}
      </div>
        ...
        createPortal(
          <div className={styles.imageModal} role="dialog" aria-modal="true" aria-label="Vista ampliada de imagen">
            ...
            <div className={styles.imageModalContent}>
              ...
              <button type="button" className={styles.imageModalClose} onClick={closeImageModal} aria-label="Cerrar">
                <span className="material-icons">close</span>
              </button>
              <img src={activeExpandedImage.src} alt={activeExpandedImage.alt} className={styles.imageModalImage} />
            </div>
          </div>,
          document.body,
        )}
        ...
        <div className={styles.deleteModal}>
          <div className={styles.modalContent}>
            ...
            <div className={styles.modalActions}>
              ...
              <button className={styles.modalDelete} onClick={handleDelete} disabled={isDeletingComment}>
                {isDeletingComment ? (
                  <>
                    <span className={styles.spinner}></span>
