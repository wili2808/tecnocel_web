...
const ReplyCard: React.FC<ReplyCardProps> = memo(
  ({ respuesta, currentUserId, currentSystemUserId, isSystemUser = false, onDelete, onModerate }) => {
      ...
      <div className={`${styles.replyCard} ${respuesta.tipo_autor === 'admin' ? styles.adminReply : ''}`}>
        <div className={styles.replyHeader}>
          <div className={styles.authorInfo}>
            ...
            {respuesta.tipo_autor === 'admin' && <span className={styles.adminBadge}>Equipo oficial</span>}
          </div>
        ...
        <div className={styles.actions}>
          {(isOwner || puedeEliminarCualquiera) && !showDeleteConfirm && (
            <button className={styles.deleteBtn} onClick={() => setShowDeleteConfirm(true)} disabled={isDeleting}>
              <span className="material-icons">delete_outline</span>
            </button>
          )}

          {isSystemUser && puedeModerar && !isOwner && onModerate && respuesta.estado === 'activo' && (
            <button className={styles.moderateBtn} onClick={() => handleModerate('oculto')}>
              <span className="material-icons">visibility_off</span>
            </button>
          )}

          {isSystemUser && puedeModerar && respuesta.estado === 'oculto' && onModerate && (
            <button className={styles.restoreBtn} onClick={() => handleModerate('activo')}>
          ...
          {showDeleteConfirm && (
            <div className={styles.deleteConfirm}>
              ...
              <button className={styles.confirmYes} onClick={handleDelete} disabled={isDeleting}>
                {isDeleting ? 'Eliminando...' : 'Sí, eliminar'}
              </button>
              <button className={styles.confirmNo} onClick={() => setShowDeleteConfirm(false)} disabled={isDeleting}>
                Cancelar
              </button>
            </div>
          )}
