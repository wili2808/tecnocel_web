...
interface CommentFormProps {
  productName: string;
  onSubmit: (data: { comentario: string; calificacion?: number; imagenes?: File[] }) => void;
  isSubmitting: boolean;
  className?: string;
}

const CommentForm: React.FC<CommentFormProps> = ({ productName, onSubmit, isSubmitting, className = '' }) => {
    ...
    <form className={`${styles.commentForm} ${className}`} onSubmit={handleSubmit}>
      ...
      <div className={styles.ratingSection}>
        ...
        <div className={styles.starRating}>
          {[1, 2, 3, 4, 5].map((star) => (
            ...
            >
              <span className="material-icons">{star <= (calificacion || 0) ? 'star' : 'star_border'}</span>
            </button>
          ))}
          {calificacion && (
      ...
      <div className={styles.commentSection}>
        ...
        <textarea
          ...
          required
        />
        <div className={styles.charCount}>{comentario.length}/2000 caracteres</div>
      </div>

      {/* Imágenes */}
      <div className={styles.imagesSection}>
        <label className={styles.label}>Imágenes (opcional, máximo 5)</label>
          ...
          <div className={styles.imagePreview}>
            {imagenes.map((file, index) => (
              <div key={index} className={styles.imageItem}>
                <img src={URL.createObjectURL(file)} alt={`Preview ${index + 1}`} className={styles.previewImage} />
                <button
                  type="button"
      ...
      <div className={styles.formActions}>
        <Button type="submit" size="xs" disabled={isSubmitting || comentario.trim().length < 10}>
          {isSubmitting ? (
            <>
