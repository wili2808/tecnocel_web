...
const Direcciones = () => {
  ...
  const handleGuardarDireccion = async (formData: DireccionFormData) => {
    ...
    try {
      ...
      if (editingDireccion) {
        await direccionService.updateDireccion(editingDireccion.id_direccion, direccionData);
      ...
      showNotification(error.response?.data?.error || 'Error al guardar dirección', 'error');
      throw error;
    }
  };
  ...
  const confirmarEliminar = async () => {
    ...
    try {
      ...
      showNotification(error.response?.data?.error || 'Error al eliminar dirección', 'error');
    } finally {
          ...
          {direcciones.map((direccion) => (
            <div key={direccion.id_direccion} className={styles.direccionCard}>
              <div className={styles.cardHeader}>
                <h3 className={styles.direccionNombre}>{direccion.nombre_direccion}</h3>
                <div className={styles.badges}>
                  {direccion.es_predeterminada && (
                    <span className={`${styles.badge} ${styles.badgePrimary}`}>Predeterminada</span>
                  )}
                  {direccion.es_facturacion && (
                    <span className={`${styles.badge} ${styles.badgeSecondary}`}>Facturación</span>
                  )}
                </div>
              </div>
              ...
              <div className={styles.cardActions}>
                <button onClick={() => handleEditarDireccion(direccion)} className={styles.btnEdit}>
                  <span className="material-icons">edit</span>
                  Editar
                </button>
                <button onClick={() => handleEliminarDireccion(direccion.id_direccion)} className={styles.btnDelete}>
        ...
        <div className={styles.deleteModal} onClick={() => setDeleteModalOpen(false)}>
          <div className={styles.deleteModalContent} onClick={(e) => e.stopPropagation()}>
            ...
            <div className={styles.deleteModalActions}>
              <button onClick={() => setDeleteModalOpen(false)} className={styles.btnSecondary}>
                Cancelar
              </button>
