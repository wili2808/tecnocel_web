...
const ProductCatalog: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (!hasInitialized) {
      ...
      if (!initializationLogsShown && import.meta.env.DEV) {
  ...
  useEffect(() => {
    ...
    if (ofertas.length === 0) {
      ...
      loadOfertas();
    } else if (!initializationLogsShown && import.meta.env.DEV) {
  ...
  const handleFiltersChange = useCallback(
    (newFilters: Partial<ProductUIFilters>) => {
      const backendFilters: any = {};

      if (newFilters.selectedDropdownCategory !== undefined) {
        backendFilters.categoria = newFilters.selectedDropdownCategory
          ? parseInt(newFilters.selectedDropdownCategory)
          : undefined;
      }

      if (newFilters.selectedDropdownBrand !== undefined) {
        backendFilters.marca = newFilters.selectedDropdownBrand
          ? parseInt(newFilters.selectedDropdownBrand)
          : undefined;
      }
